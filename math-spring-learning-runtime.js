(function (root, factory) {
  ...
  if (root) root.STUDY_MATH_SPRING_LEARNING_RUNTIME = api;
})(typeof window !== "undefined" ? window : globalThis, function () {
  "use strict";

  const FACTORY_VERSION = "mathSpringLearningRuntimeFactoryV1";

  function createRuntime(config = {}) {
  const schema = config.schema;
  const model = config.model;
  const learningContent = config.learningContent;
  if (!schema || !model || !learningContent) {
    throw new Error("Math spring learning schema, model, and content are required");
  }

  const VERSION = String(config.version || "mathSpringLearningV1");
  const SCOPE_ID = String(config.scopeId || "");
  const CURRICULUM_VERSION = String(config.curriculumVersion || "");
  const CLOUD_STATE_KEY = String(config.cloudKey || VERSION);
  const LOCAL_STORAGE_KEY = String(config.localStorageKey || "");
  const CYCLE_PREFIX = String(config.cyclePrefix || "math-cycle");
  const SUBMISSION_PREFIX = String(config.submissionPrefix || CYCLE_PREFIX.replace(/-cycle$/, "-submit"));
  const EVENT_NAME = String(config.eventName || "");
  const STAGES = Object.freeze([...(config.stages || ["BASIC", "A1", "A2", "A3", "A4", "A5"])]);
  const validator = config.validator || schema;
  const independentCheckPool = config.independentCheckPool || null;
  const answerTypeHandlers = Object.freeze({ ...(config.answerTypeHandlers || {}) });
  const completionRules = Object.freeze({ ...(config.completionRules || {}) });
  const stageProgressionRules = Object.freeze({ ...(config.stageProgressionRules || {}) });
  const onCycleCompleted = typeof config.onCycleCompleted === "function"
    ? config.onCycleCompleted
    : null;
  ...
  const CONCEPT_IDS = Object.freeze([
    ...(Array.isArray(config.conceptIds)
      ? config.conceptIds
      : learningContent.CONCEPTS.map((concept) => concept.conceptId)),
  ]);
  ...
  function nextStage(stage) {
    if (typeof stageProgressionRules.nextStage === "function") {
      const selected = stageProgressionRules.nextStage(stage, { stages: [...STAGES] });
  ...
  function previousStage(stage) {
    if (typeof stageProgressionRules.previousStage === "function") {
      const selected = stageProgressionRules.previousStage(stage, { stages: [...STAGES] });
      if (STAGES.includes(selected)) return selected;
    }
    const index = stageIndex(stage);
  ...
  function problemsFor(conceptId, stage, purpose = PURPOSES.LEARNING) {
    const source = purpose === PURPOSES.INDEPENDENT_CHECK && independentCheckPool
      ? independentCheckPool
      : model;
    if (typeof source === "function") {
      const problems = source({ conceptId, stage, purpose });
      return Array.isArray(problems) ? problems : [];
    }
    if (source && typeof source.getProblems === "function") {
      const problems = source.getProblems(conceptId, stage, { purpose });
      return Array.isArray(problems) ? problems : [];
    }
    return [];
  }
  ...
  function problemById(problemId) {
    if (model?.problemsById?.[problemId]) return model.problemsById[problemId];
    if (independentCheckPool?.problemsById?.[problemId]) {
      return independentCheckPool.problemsById[problemId];
    }
    if (independentCheckPool && typeof independentCheckPool.getProblem === "function") {
      return independentCheckPool.getProblem(problemId) || null;
    }
    return null;
  }

  function chooseProblem(state, conceptId, stage, usedIds, usedStructures, usedSolutions, options = {}) {
    const pool = problemsFor(conceptId, stage, options.purpose);
  ...
  function buildSpringCycle(stateInput, options = {}) {
    ...
    state.pendingIndependentChecks.forEach((check) => {
      ...
      let problem = chooseProblem(
        ...
        usedSolutions,
        {
          ...
          purpose: PURPOSES.INDEPENDENT_CHECK,
        }
      );
      if (!problem) {
    ...
    selectedTargets.forEach(({ conceptId, stage }) => {
      ...
      const pool = problemsFor(conceptId, stage);
  ...
  function startCycle(stateInput, options = {}) {
    ...
    state.activeLearningCycleId = `${CYCLE_PREFIX}-${state.activeCycleNumber}-${idFactory()}`;
  ...
  function currentProblem(state) {
    ...
    return item ? problemById(item.problemId) : null;
  }
  ...
  function useHint(stateInput, now = Date.now) {
    ...
    const item = currentItem(state);
    if (
      item?.purpose === PURPOSES.INDEPENDENT_CHECK
      && problem.independentCheckPolicy?.hintDisclosure === "LOCKED_DURING_INDEPENDENT_CHECK"
    ) {
      return { state, hint: null, level: 0, reason: "INDEPENDENT_CHECK_LOCKED" };
    }
  ...
  function submissionId(problemId, now, idFactory) {
    return `${SUBMISSION_PREFIX}-${problemId}-${typeof now === "function" ? now() : Date.now()}-${idFactory()}`;
  }
  ...
  function queueAlternatePractice(state, problem) {
    ...
    const alternative = problemsFor(problem.conceptId, problem.stage).find((candidate) => (
      candidate.problemId !== problem.problemId
      && candidate.reasoningGoals.some((goal) => problem.reasoningGoals.includes(goal))
  ...
  function submitAnswer(stateInput, answer, options = {}) {
    ...
    const handler = answerTypeHandlers[problem.answerType];
    ...
    if (typeof handler === "function") {
      result = handler(problem, answer, validationContext);
    } else if (handler && typeof handler.evaluateAnswer === "function") {
      result = handler.evaluateAnswer(problem, answer, validationContext);
    } else if (typeof validator === "function") {
      result = validator(problem, answer, validationContext);
    } else if (validator && typeof validator.evaluateAnswer === "function") {
      result = validator.evaluateAnswer(problem, answer, validationContext);
    } else {
      result = schema.evaluateAnswer(problem, answer);
    }
    const record = {
  ...
  function revealSolution(stateInput, options = {}) {
    ...
    const problem = currentProblem(state);
    const item = currentItem(state);
    const finalized = problem ? getFinalizedAttempt(state, problem.problemId) : null;
    const independentAfterFinal = item?.purpose === PURPOSES.INDEPENDENT_CHECK
      && problem?.independentCheckPolicy?.solutionDisclosure === "AFTER_FINAL";
    if (!problem) {
      return { state, revealed: false, reason: "FINAL_LOCKED_OR_MISSING" };
    }
    const key = attemptKey(state, problem.problemId);
    if (independentAfterFinal) {
      if (!finalized) {
        return { state, revealed: false, reason: "INDEPENDENT_CHECK_LOCKED" };
      }
      ...
      state.revision += 1;
      state.updatedAt = timestamp(now);
      return { state, revealed: true, problem: clone(problem), postFinal: true };
    }
    if (finalized) {
      return { state, revealed: false, reason: "FINAL_LOCKED_OR_MISSING" };
    }
  ...
  function completeLearningTargets(state, now) {
    Object.entries(state.cycleTargets || {}).forEach(([conceptId, targetStage]) => {
      ...
      const configuredRequired = typeof completionRules.requiredLearningItems === "function"
        ? completionRules.requiredLearningItems({
          conceptId,
          stage: targetStage,
          targetItems: clone(targetItems),
          defaultCount: problemsFor(conceptId, targetStage).length,
        })
        : null;
      const requiredLearningItems = Number.isFinite(configuredRequired)
        ? Math.max(0, Number(configuredRequired))
        : problemsFor(conceptId, targetStage).length;
      ...
      if (typeof completionRules.canCompleteTarget === "function" && !completionRules.canCompleteTarget({
        state: clone(state),
        conceptId,
        stage: targetStage,
        targetItems: clone(targetItems),
  ...
  function applyIndependentResults(state) {
    state.independentPracticeResults.forEach((result) => {
      ...
      const passed = typeof completionRules.independentCheckPassed === "function"
        ? Boolean(completionRules.independentCheckPassed({
          state: clone(state),
          result: clone(result),
          check: clone(check),
        }))
        : result.passed;
      check.status = passed ? "VERIFIED" : "RETRY_REQUIRED";
